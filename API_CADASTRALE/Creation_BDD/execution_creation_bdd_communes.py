...
class Execution_creation_bdd_communes:
    def __init__(
        self, chemin_local: str, date: str = "latest", departements: list = []
    ):
    ...
    def applique(self):
        ...
        for commune1 in communes:
            if France_Entiere or (commune1.departement() in self.departements):
        ...
        if France_Entiere == True:
            liste_pos = list(range(len(data)))
        else:
            liste_pos = []
            for ligne in matchs:
